...
struct BoundingBox {
	 ...
	 * Full constructor.
	 */
	BoundingBox(const Point& _min, const Point& _max) : min(_min), max(_max) { }

	/** Automatically build the BoundingBox from a bunch of objects.
	 * NAIVE: can be optimized by iteratively checking the coordinates of each object
	 */
	BoundingBox(std::vector<BoundingBox> &boxes)
	{
		using std::vector;
	 ...
	 * Basically expands the box up to the given point
	 */

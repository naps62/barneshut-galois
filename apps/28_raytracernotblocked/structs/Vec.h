...
struct Vec {
	...
	Vec  min(const Vec& v)   const { return Vec(std::min(x, v.x), std::min(y, v.y), std::min(z, v.z)); }
	Vec  max(const Vec& v)   const { return Vec(std::max(x, v.x), std::max(y, v.y), std::max(z, v.z)); }
	double dot(const Vec &b) const { return x * b.x + y * b.y + z * b.z; }
	double max_coord()       const { return std::max(x, std::max(y, z)); }
	...
	double distanceSquared (const Vec& v) const { return (*this - v).lengthSquared(); }
	double distance        (const Vec& v) const { return sqrt(distanceSquared(v)); }
